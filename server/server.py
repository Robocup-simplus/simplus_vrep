            ...
            while not isExit:
                ...
                response = stub.Action(
                    simplus_pb2.Observations(
                        ...
                        robots=[simplus_pb2.Observation(
                            camera=simplus_pb2.Image(w=image[1], h=image[2],
                                                     raw=array.array('B', image_array).tobytes()),
                            colors=[simplus_pb2.Pixel(r=colors[i][0], g=colors[i][1], b=colors[i][2]) for i in
                                    range(3)],
                            distances=[simplus_pb2.Proximity(detected=proxim[i][0], distance=proxim[i][1]) for i in
                                       range(8)],
                            pos=simplus_pb2.Position(x=pos[0], y=pos[1], z=pos[2], roll=pos[3], pitch=pos[4],
                                                     yaw=pos[5],
                                                     gps_enabled=ra.gps_enabled),
                            thermalCamera=simplus_pb2.Image(w=thermal_image[1], h=thermal_image[2],
                                                            raw=array.array('B', thermal_image_array).tobytes())
                        ) for i in range(1)]
                    )
                )
                ...
                for res in response.commands:
                    ...
                    for action in res.actions:
                        ...
                        action_score = sa.callAction(action.type, robot_pose[0], robot_pose[1], robot_pose[2],
                                                     team_score)
